...
export async function handleDashboardApi(method, subpath, body, req, res) {
  ...
  // ─── AI Bash & Project Generator ──────────────────────
  // POST /system/ai/generate-bash - Generate bash command using AI
  if (subpath === '/system/ai/generate-bash' && method === 'POST') {
...
- Prefer safe commands like: ls, cat, grep, ps, df, du, ping, curl, date, whoami, find, wc
...
If the request is unsafe or could be destructive, respond with: UNSAFE: <reason>`;
      ...
      const command = await queryLocalLLM(prompt, 'llama3.2', { temperature: 0.3, maxTokens: 200 });
      const trimmed = command.trim();
      
      if (trimmed.startsWith('UNSAFE:')) {
        return json(res, 400, { success: false, error: trimmed.substring(7).trim() });
      }
      
      return json(res, 200, { success: true, command: trimmed });
    } catch (err) {
      return json(res, 500, { success: false, error: err.message });
    }
  }

  // POST /system/ai/generate-project - Generate project structure using AI
  if (subpath === '/system/ai/generate-project' && method === 'POST') {
    if (!body.description || !body.name) return json(res, 400, { error: 'description and name are required' });
    try {
      const prompt = `You are a helpful assistant that generates project structures.

Create a project structure for: "${body.description}"
Project name: "${body.name}"

Generate a tree-like structure showing all files and directories.
Format: Show as a tree with files (📄) and directories (📁)
Include:
1. Main entry files
...
Only show the structure, no explanations.`;

      const structure = await queryLocalLLM(prompt, 'llama3.2', { temperature: 0.4, maxTokens: 1000 });
      return json(res, 200, { success: true, structure: structure.trim() });
    } catch (err) {
      return json(res, 500, { success: false, error: err.message });
    }
  }

  // POST /system/ai/create-project - Create project files using AI
  if (subpath === '/system/ai/create-project' && method === 'POST') {
    if (!body.description || !body.name || !body.workspace) {
      return json(res, 400, { error: 'description, name, and workspace are required' });
    }
    try {
      const projectPath = `${body.workspace}/${body.name}`;
      const mkdirResult = await executeBash(`mkdir -p "${projectPath}"`, { timeout: 5000 });
      
      if (mkdirResult.exitCode !== 0) {
        return json(res, 500, { success: false, error: 'Failed to create project directory' });
      }

      // Generate and create basic structure
      const files = [];
      
      // Create README.md
      const readmeContent = `# ${body.name}\n\n${body.description}\n\nGenerated by WindsurfPoolAPI AI\n`;
      await executeBash(`echo "${readmeContent.replace(/"/g, '\\"')}" > "${projectPath}/README.md"`, { timeout: 5000 });
      ...
      return json(res, 200, { 
        ...
        projectPath, 
        files,
        message: `Project "${body.name}" created successfully at ${projectPath}`
      });
    } catch (err) {
      return json(res, 500, { success: false, error: err.message });
    }
